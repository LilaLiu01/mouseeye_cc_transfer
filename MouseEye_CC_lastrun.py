...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/seanliu/Documents/AMC_Lab/MouseEye/MouseEye_TunnelVision_REP_ContextualCueing_fromscratch/MouseEye_CC_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    InstrMainExp = visual.TextStim(win=win, name='InstrMainExp',
        text='Now we begin the real experiment!\n\nThe task is the same as the practice you just did: click the white dot to start, then move your mouse around to find letter T. This experiment takes about 80 minutes. There are 24 blocks. At the end of each block, you will see your response accuracy and can take a short break. \n\nPlease respond as accurately as quickly as possible, and keep the same pose during experiment.\n\nReady? Press spacebar to start!',
        font='Arial',
        pos=(0, 0), height=24, wrapWidth=None, ori=0, 
        color='white', colorSpace='rgb', opacity=1, 
        languageStyle='LTR',
    ...
    textBreak = visual.TextStim(win=win, name='textBreak',
        text='There are 6 blocks altogether. The number of remaining blocks is:\n\n\nPress spacebar to continue. ',
        font='Arial',
    ...
    # --- Initialize components for Routine "InstrTesting" ---
    InstrTestingPhase = visual.TextStim(win=win, name='InstrTestingPhase',
        text='Thank you for completing the training!\n\nNow you will be instructed into the test where you can see the whole scene. You are required to find the letter T and report the direction of its tip using the A key (tip of T facing left) and S key (tip of T facing right).\n\nPress the spacebar to enter the test once you are already!',
        font='Arial',
        pos=(0, 0), height=24.0, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=1.0, 
        languageStyle='LTR',
        depth=0.0);
    testPhase_key = keyboard.Keyboard()
    ...
    CenterAperture_3 = visual.ImageStim(
        ...
        flipHoriz=False, flipVert=False,
        texRes=512, interpolate=True, depth=-15.0)
    key_respMain_3 = keyboard.Keyboard()
    ...
    # --- Initialize components for Routine "fixRT" ---
    # Run 'Begin Experiment' code from fixRecogTask_code
    # initial fixation size
    fixsize = 28
    # intial hint text location
    hintlocation = -255;
    fixPointRT = visual.ImageStim(
        win=win,
        name='fixPointRT', 
        image='fixation.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=1.0,
    ...
    CenterAperture_2 = visual.ImageStim(
        ...
        flipHoriz=False, flipVert=False,
        texRes=512, interpolate=True, depth=-15.0)
    key_respMain_2 = keyboard.Keyboard()
    ...
    # Run 'Begin Routine' code from skip_offline_3
    #skip this component offline
    continueRoutine = True
    ...
    for thisTrialsPractice in trialsPractice:
        ...
        # keep track of which components have finished
        trialComponents = [background, imageT, imageL1, imageL2, imageL3, imageL4, imageL5, imageL6, imageL7, imageL8, imageL9, imageL10, imageL11, imageAperture, CenterAperture, searchHint, mouseeye, key_respMain]
    ...
    # --- Prepare to start Routine "InstrTesting" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('InstrTesting.started', globalClock.getTime())
    testPhase_key.keys = []
    testPhase_key.rt = []
    _testPhase_key_allKeys = []
    # keep track of which components have finished
    InstrTestingComponents = [InstrTestingPhase, testPhase_key]
    for thisComponent in InstrTestingComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # *InstrTestingPhase* updates
        
        # if InstrTestingPhase is starting this frame...
        if InstrTestingPhase.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            InstrTestingPhase.frameNStart = frameN  # exact frame index
            InstrTestingPhase.tStart = t  # local t and not account for scr refresh
            InstrTestingPhase.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(InstrTestingPhase, 'tStartRefresh')  # time at next scr refresh
            # update status
            InstrTestingPhase.status = STARTED
            InstrTestingPhase.setAutoDraw(True)
        
        # if InstrTestingPhase is active this frame...
        if InstrTestingPhase.status == STARTED:
            # update params
            pass
        
        # *testPhase_key* updates
        waitOnFlip = False
        
        # if testPhase_key is starting this frame...
        if testPhase_key.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
            # keep track of start time/frame for later
            testPhase_key.frameNStart = frameN  # exact frame index
            testPhase_key.tStart = t  # local t and not account for scr refresh
            testPhase_key.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(testPhase_key, 'tStartRefresh')  # time at next scr refresh
            # update status
            testPhase_key.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(testPhase_key.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(testPhase_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if testPhase_key.status == STARTED and not waitOnFlip:
            theseKeys = testPhase_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _testPhase_key_allKeys.extend(theseKeys)
            if len(_testPhase_key_allKeys):
                testPhase_key.keys = _testPhase_key_allKeys[-1].name  # just the last key pressed
                testPhase_key.rt = _testPhase_key_allKeys[-1].rt
                testPhase_key.duration = _testPhase_key_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # --- Ending Routine "InstrTesting" ---
    for thisComponent in InstrTestingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('InstrTesting.stopped', globalClock.getTime())
    # the Routine "InstrTesting" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisTestingtrial in Testingtrials:
        ...
        key_respMain_3.keys = []
        key_respMain_3.rt = []
        _key_respMain_3_allKeys = []
        # keep track of which components have finished
        displayTestingPhaseComponents = [background_3, imageT_3, imageL1_3, imageL2_3, imageL3_3, imageL4_3, imageL5_3, imageL6_3, imageL7_3, imageL8_3, imageL9_3, imageL10_3, imageL11_3, imageAperture_3, CenterAperture_3, key_respMain_3]
        ...
        while continueRoutine:
            ...
            # *key_respMain_3* updates
            waitOnFlip = False
            
            # if key_respMain_3 is starting this frame...
            if key_respMain_3.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                key_respMain_3.frameNStart = frameN  # exact frame index
                key_respMain_3.tStart = t  # local t and not account for scr refresh
                key_respMain_3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_respMain_3, 'tStartRefresh')  # time at next scr refresh
                # update status
                key_respMain_3.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_respMain_3.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_respMain_3.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_respMain_3.status == STARTED and not waitOnFlip:
                theseKeys = key_respMain_3.getKeys(keyList=['y','n'], ignoreKeys=["escape"], waitRelease=False)
                _key_respMain_3_allKeys.extend(theseKeys)
                if len(_key_respMain_3_allKeys):
                    key_respMain_3.keys = _key_respMain_3_allKeys[0].name  # just the first key pressed
                    key_respMain_3.rt = _key_respMain_3_allKeys[0].rt
                    key_respMain_3.duration = _key_respMain_3_allKeys[0].duration
                    # was this correct?
                    if (key_respMain_3.keys == str(corrAnswer)) or (key_respMain_3.keys == corrAnswer):
                        key_respMain_3.corr = 1
                    else:
                        key_respMain_3.corr = 0
                    # a response ends the routine
                    continueRoutine = False
        ...
        thisExp.addData('searchsize',searchsize);
        # check responses
        if key_respMain_3.keys in ['', [], None]:  # No response was made
            key_respMain_3.keys = None
            # was no response the correct answer?!
            if str(corrAnswer).lower() == 'none':
               key_respMain_3.corr = 1;  # correct non-response
            else:
               key_respMain_3.corr = 0;  # failed to respond (incorrectly)
        # store data for Testingtrials (TrialHandler)
        Testingtrials.addData('key_respMain_3.keys',key_respMain_3.keys)
        Testingtrials.addData('key_respMain_3.corr', key_respMain_3.corr)
        if key_respMain_3.keys != None:  # we had a response
            Testingtrials.addData('key_respMain_3.rt', key_respMain_3.rt)
            Testingtrials.addData('key_respMain_3.duration', key_respMain_3.duration)
    ...
    RTtrials = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Seq_recogtask.xlsx'),
    ...
    for thisRTtrial in RTtrials:
        ...
        # --- Prepare to start Routine "fixRT" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('fixRT.started', globalClock.getTime())
        # Run 'Begin Routine' code from fixRecogTask_code
        win.mouseVisible = True; # show cursor
        # fixation Hint
        if phase == 'practice':
            fixationStartTime = 4;
        ...
        # hint text location rescale
        hintlocation = -255*expInfo['ratio_monitor2CRT'];
        fixPointRT.setSize((fixsize, fixsize))
        startRT_key.keys = []
        startRT_key.rt = []
        _startRT_key_allKeys = []
        # keep track of which components have finished
        fixRTComponents = [fixPointRT, startRT_key]
        for thisComponent in fixRTComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if fixPointRT is starting this frame...
            if fixPointRT.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fixPointRT.frameNStart = frameN  # exact frame index
                fixPointRT.tStart = t  # local t and not account for scr refresh
                fixPointRT.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fixPointRT, 'tStartRefresh')  # time at next scr refresh
                # update status
                fixPointRT.status = STARTED
                fixPointRT.setAutoDraw(True)
            
            # if fixPointRT is active this frame...
            if fixPointRT.status == STARTED:
                # update params
                pass
            ...
            # if startRT_key is starting this frame...
            if startRT_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                startRT_key.frameNStart = frameN  # exact frame index
                startRT_key.tStart = t  # local t and not account for scr refresh
                startRT_key.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(startRT_key, 'tStartRefresh')  # time at next scr refresh
                # update status
                startRT_key.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(startRT_key.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(startRT_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if startRT_key.status == STARTED and not waitOnFlip:
                theseKeys = startRT_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _startRT_key_allKeys.extend(theseKeys)
                if len(_startRT_key_allKeys):
                    startRT_key.keys = _startRT_key_allKeys[-1].name  # just the last key pressed
                    startRT_key.rt = _startRT_key_allKeys[-1].rt
                    startRT_key.duration = _startRT_key_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # --- Ending Routine "fixRT" ---
        for thisComponent in fixRTComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('fixRT.stopped', globalClock.getTime())
        # Run 'End Routine' code from fixRecogTask_code
        mouselocation = [0, 0];
        thisExp.addData('fixationSize',fixsize)
        # the Routine "fixRT" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        key_respMain_2.keys = []
        key_respMain_2.rt = []
        _key_respMain_2_allKeys = []
        # keep track of which components have finished
        displayRTComponents = [background_2, imageT_2, imageL1_2, imageL2_2, imageL3_2, imageL4_2, imageL5_2, imageL6_2, imageL7_2, imageL8_2, imageL9_2, imageL10_2, imageL11_2, imageAperture_2, CenterAperture_2, key_respMain_2]
        ...
        while continueRoutine:
            ...
            # *key_respMain_2* updates
            waitOnFlip = False
            
            # if key_respMain_2 is starting this frame...
            if key_respMain_2.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                key_respMain_2.frameNStart = frameN  # exact frame index
                key_respMain_2.tStart = t  # local t and not account for scr refresh
                key_respMain_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_respMain_2, 'tStartRefresh')  # time at next scr refresh
                # update status
                key_respMain_2.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_respMain_2.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_respMain_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_respMain_2.status == STARTED and not waitOnFlip:
                theseKeys = key_respMain_2.getKeys(keyList=['y','n'], ignoreKeys=["escape"], waitRelease=False)
                _key_respMain_2_allKeys.extend(theseKeys)
                if len(_key_respMain_2_allKeys):
                    key_respMain_2.keys = _key_respMain_2_allKeys[0].name  # just the first key pressed
                    key_respMain_2.rt = _key_respMain_2_allKeys[0].rt
                    key_respMain_2.duration = _key_respMain_2_allKeys[0].duration
                    # was this correct?
                    if (key_respMain_2.keys == str(corrAnswer)) or (key_respMain_2.keys == corrAnswer):
                        key_respMain_2.corr = 1
                    else:
                        key_respMain_2.corr = 0
                    # a response ends the routine
                    continueRoutine = False
        ...
        thisExp.addData('searchsize',searchsize);
        # check responses
        if key_respMain_2.keys in ['', [], None]:  # No response was made
            key_respMain_2.keys = None
            # was no response the correct answer?!
            if str(corrAnswer).lower() == 'none':
               key_respMain_2.corr = 1;  # correct non-response
            else:
               key_respMain_2.corr = 0;  # failed to respond (incorrectly)
        # store data for RTtrials (TrialHandler)
        RTtrials.addData('key_respMain_2.keys',key_respMain_2.keys)
        RTtrials.addData('key_respMain_2.corr', key_respMain_2.corr)
        if key_respMain_2.keys != None:  # we had a response
            RTtrials.addData('key_respMain_2.rt', key_respMain_2.rt)
            RTtrials.addData('key_respMain_2.duration', key_respMain_2.duration)
